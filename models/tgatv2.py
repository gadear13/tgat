...
class TGATCell(torch.nn.Module):
    def __init__(
        ...
        self.conv1 = GATv2Conv(in_channels, hid_channels, heads=heads, concat=concat, dropout=dropout,
                               share_weights=share_weights, **kwargs)
        self.conv2 = GATv2Conv(in_channels, hid_channels, heads=heads, concat=concat, dropout=dropout,
                               share_weights=share_weights, **kwargs)

    def forward(self, x, edge_index, edge_weight, h):
        # Convolution
        f1 = torch.sigmoid(self.conv1(x, edge_index))
        f2 = torch.sigmoid(self.conv2(x, edge_index))

        # r, u
        cat1 = torch.concat([f1, h], dim=1)
        ru = torch.sigmoid(self.linear1(cat1))
        r, u = torch.chunk(ru, chunks=2, dim=1)

        # c
        cat2 = torch.concat([f2, r * h], dim=1)
